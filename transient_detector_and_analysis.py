...
for filename in glob.glob('*' + string + 'string.mat'):
    ...
    for i in range(len(file_onsets2)):
        ...
        transients_array[0][4] = filename.split('_')[4]
        transients_array[0][5] = filename.split('_')[5]
        transients_array[0][6] = int(file_onsets2[i]*sr)
        transients_array[0][7] = transient_end
    ...
    if len(file_onsets1) != len(file_onsets2):
        ...
        if len(std_peaks_array1) != len(std_peaks_array2):
            print("Standard deviation matrix are not the same size")
    ...
    if display == 'ON':
        ...
        ax[1].scatter((np.arange(len(np.diff(sig)))/sr)[peaks1],
                      ...
                      c='purple', marker='*')
        ax[1].plot(np.arange(len(pp_height))/sr, pp_height, color='yellow',
