...
for filename in glob.glob('*' + string + 'string.mat'):
    ...
    # Detect peaks
    peaks1, _ = scipy.signal.find_peaks(abs_sig_diff, prominence=(0.005,5),
                                       distance=200)
    peaks2, _ = scipy.signal.find_peaks(abs_pick_diff, prominence = (0.005,5),distance = 200)
    ...
    for i in range(len(file_onsets1)):
        ...
        transients_array[0][6] = int(file_onsets1[i]*sr)
    ...
    if display == 'ON':
        ...
        ax[1].scatter((np.arange(len(np.diff(sig)))/sr)[peaks1],
                      np.abs(np.diff(sig))[peaks1], label='Detected peaks',
                      c='r', marker='*')
        ax[1].plot(np.arange(len(pp_height))/sr, pp_height, color='g',
                    label='Maximum height for peak detection')
        ax[2].plot(np.arange(len(nb_peaks1))/sr, nb_peaks1, color='g',
        ...
        ax[2].vlines(file_onsets1, 0, 40, color='r', alpha=0.9,
                      linestyle='--')
        ax[0].plot(np.arange(len(pickup))/sr, pickup, label='Signal')
        ax[0].set(title=filename)
        ax[1].plot(np.arange(len(abs_pick_diff))/sr, abs_pick_diff,
                    label='First derivative signal (absolute value) pickup')
        ax[1].scatter((np.arange(len(np.diff(pickup)))/sr)[peaks2],
                      np.abs(np.diff(pickup))[peaks2], label='Detected peaks', marker='*')
        ax[2].plot(np.arange(len(nb_peaks2))/sr, nb_peaks2,
        ...
        ax[3].plot(np.arange(len(spf_array1))/sr, spf_array1, color='orange',
                    label='Slipping phase frequency (SPF)')
        ax[3].plot(np.arange(len(spf_array2))/sr, spf_array2,
                    label='Slipping phase frequency (SPF) pickup')
        ax[4].plot(np.arange(len(std_peaks_array2))/sr, std_peaks_array2, label='Standard deviation (STD) pickup')
